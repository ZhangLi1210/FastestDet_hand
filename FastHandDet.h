//
// Created by lizhang on 2023/11/20.
//

#ifndef FASTHANDDET_H
#define FASTHANDDET_H
...
class FastHandDet {
public:
    explicit FastHandDet(const std::string& FastDetHand_engine_file_path,int input_width, int input_length, float score_threshold_ = 0.7, float iou_threshold_ = 0.3);
    ~FastHandDet();
    ...
    void                 detect(cv::Mat im, std::vector<TargetBox> &hand_list);
    ...
    std::vector<void*>   FastHandDet_host_ptrs;  //模型的的CPU指针
    std::vector<void*>   FastHandDet_device_ptrs; //模型的GPU指针  对于每一个输入, 都需要给它分配一个GPU的内存空间,用于将CPU数据传给GPU
...
private:
    ...
    nvinfer1::ICudaEngine*       engine_FastDetHand  = nullptr;
    nvinfer1::IExecutionContext* context_FastDetHand = nullptr;

    cudaStream_t                 stream  = nullptr;
    Logger                       gLogger{nvinfer1::ILogger::Severity::kERROR};

    float * FastHandDet_input_blob;
